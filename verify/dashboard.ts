...
try {
	// ---------- Order detail: page shim on the client ----------
	{
		...
		await page.waitForSelector('sk-island[data-hydrated]', { timeout: 4000 }).catch(() => {});
	...
	// ---------- Dashboard sidebar SPA nav + analytics visible chart ----------
	{
		...
		const chart = page.locator('sk-island[data-strategy="visible"]');
		await sleep(300);
		...
		await page.waitForSelector('sk-island[data-strategy="visible"][data-hydrated]', { timeout: 3000 }).catch(() => {});
