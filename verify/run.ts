...
//
//   node verify/run.ts                 # build fresh, serve, run all checks
//   node verify/run.ts --no-build      # reuse the existing playground build
//   node verify/run.ts --only=lakes,browser
//   PORT=3060 node verify/run.ts
//
...
const CHECKS: Array<[file: string, needsServer: boolean, note: string]> = [
	...
	['defer-hydrate.ts', true, 'defer+hydrate coalesce, mismatch visible, props/preload'],
...
if (!noBuild) {
	...
	const playground = join(repo, 'playground');
...
for (const [file, needsServer, note] of toRun) {
	...
	const args = needsServer ? [`verify/${file}`, BASE] : [`verify/${file}`];
