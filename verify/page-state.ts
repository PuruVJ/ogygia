...
//
//   node verify/page-state.ts http://localhost:3051
//
...
try {
	...
	await page.waitForSelector('[data-pageurl-probe][data-hydrated], ogygia-region[data-hydrated] [data-pageurl-probe]', {
		timeout: 5000
	}).catch(() => {});
	...
	check('page.url.pathname is non-empty inside island', pathname.length > 0, JSON.stringify(pathname));
	check('page.url.pathname matches order route', pathname === '/dashboard/orders/5', pathname);
	check('page.url.href includes pathname', href.includes('/dashboard/orders/5'), href.slice(0, 80));
	...
	await page.waitForFunction(
		() => document.querySelector('[data-pageurl-probe] [data-pathname]')?.textContent?.trim() === '/dashboard/orders/6',
		{ timeout: 5000 }
	).catch(() => {});
	...
	check('SPA nav updates page.url.pathname inside island', pathname2 === '/dashboard/orders/6', pathname2);
	check('SPA nav updates page.params.id inside island', paramId2 === '6', paramId2);
	check('SPA nav kept marker (no full reload)', (await page.evaluate(() => window.__marker)) === m1);

	// Query string via FilterBar goto on list page
	await page.goto(base + '/dashboard/orders?status=shipped', { waitUntil: 'networkidle' });
	...
	check('orders list still loads after page-state checks', (await page.locator('[data-filterbar]').count()) >= 1);
...
console.log(`\n${failures === 0 ? 'ALL PAGE-STATE CHECKS PASSED' : failures + ' PAGE-STATE CHECK(S) FAILED'}`);
