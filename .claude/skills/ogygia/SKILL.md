...
| `wake` | When does JS run (static) / when does the HTML fetch (deferred, live)? | `'load'` · `'idle'` · `'visible'` · `'interaction'` (island: first pointer/key/focus inside wakes it; deferred: an **on-demand hole** — first hover/focus/touch inside fetches it, morphed in) · `'(media query)'` · `'none'` (frozen — a lake) |
...
| A whole app section owned by another team/deploy | federation: `federate({ peers })` then `'/cms/[...rest]': mount(cms)` in the shell's routes table; the MFE `federate({ expose })`s its router | iframes, module federation |
| A/B test, rollout, kill switch, or flag, zero client JS | `flag()` from `'ogygia/flag'` (read by calling it), `flag.pick({...})` as a page slot; `decide({ source })` for OpenFeature | client-side bucketing / a vendor SDK in the bundle |
...
- `ogygia_fragment({ origin })` — probe a federated app: fetches its unsigned `__catalog` widget manifest (names + props) and reports whether the fragment endpoint verifies signatures (401 without a signature = signed, 200 = OPEN). Use it when wiring a shell to a peer, before writing `mount(peer)` / `peer.page()` / `peer.widget()` code.
...
6. **Module-level `$state` in a `.svelte.ts` already shares across islands** with zero ceremony (one client chunk = one instance). Reach for `wire` only when the instance is born on the server and handed down as a prop.
   - **`requestEvent()` from `'ogygia'`** reads Kit's `RequestEvent` (`locals`, `cookies`, `url`) inside ANY region's component during SSR — a server island on its endpoint, a lake, an island's server pass — without importing `$app/server` (which Kit's client guard rejects in a component shared with a csr=true page) and without a remote function. Call it during component init, BEFORE the first `await`. Returns `null` in the browser. A server island's own request has no page URL — pass the page's locale/id as a prop when the hole needs it.
   - **`keepFallback()` from `'ogygia'`**: a server island saying "the page's `ogygiaFallback` is right for this visitor" — the endpoint answers 204 (a batch parcel carries a marker), the runtime keeps the fallback DOM and marks the region done: no bytes, no swap. Call it as soon as the hole knows (e.g. `if (!user) keepFallback()`); it throws a signal, nothing after it runs. The shape for personal holes whose fallback is the anonymous markup: the majority of visitors cost 204s, only the logged-in ones a render.
7. `page.data` works inside islands: `$app/state`/`$app/stores` resolve to shims fed by a document-level devalue seed the handle injects (Kit alone leaves them empty under csr=false). The seed ships only when an island reads `$page`, and only the top-level `page.data` keys the page's islands read (the build pins `page.data.x` reads per island; a read it cannot pin — `page.data` passed whole, aliased, spread, indexed dynamically — ships everything). Read literal keys to keep the seed small. On a Kit-booted (csr=true) document the shims read Kit's REAL `page` instead, through the kit-world thread (two lines appended to Kit's client entry publish it on a global symbol) — so a component shared between an island and a Kit page reads correct `page.data` in BOTH worlds. `$app/navigation` inside islands: prefer `'ogygia/app'` (`goto`, `invalidate`, `invalidateAll`, `preloadData`, `beforeNavigate`, …).
...
- **CSS travels with the region**: the page never imported the component, so its scoped stylesheet rides the response and is hoisted to `<head>`; the runtime never paints unstyled. Kit's `inlineStyleThreshold` governs this channel too: a region sheet under it ships as `<style data-ogygia-region-css="href">` (no render-blocking request) and is hoisted/deduped by that href like a link; unset (Kit's default) = links as always.
...
- `interaction` on an island: zero JS until first pointer/key/focus **inside** the region. The waking click is captured and replayed; typing survives; hover prefetches the chunk. Caveat: the replayed event is not a trusted gesture — `window.open`/clipboard/fullscreen will be blocked; detect with `hydratedBy() === 'interaction'` and render e.g. a real `<a target="_blank">`.
- `interaction` on a server island = an **on-demand hole**: zero bytes until the visitor shows intent inside it — the first hover (`pointerover`, which bubbles through the boxless `display:contents` wrapper from a child), focus, touch, or key — then ONE fetch, **morphed** in so what the visitor already opened in the static fallback stays open (no click capture/replay — the fallback handles the gesture natively). Hover is the trigger, so the HTML is usually there before the visitor reaches a submenu. To have it there BEFORE the first hover, add `prefetch: 'idle'` (or `load` / `visible` / a media query — deferred-only, never `interaction`): the HTML is warmed into the frame store at that schedule and the hover swaps from it with no request. The shape for a mega menu: fallback = the menu's L1/L2 (today's SSR HTML, byte-identical), hole = the same menu with L3/L4 rendered on the server — no L4 in the initial HTML, no menu JS, no feed download in the browser.
- Nesting — one rule, **closest marked parent decides**: island-in-island shares the parent's JS (one tree, child's schedule ignored, dev warns); lake-in-island stays static; island-in-lake wakes on its own; server-island-in-island renders inline (`deferred` ignored); an island inside a server island's `ogygiaFallback` is a full region that wakes on its own (the fallback belongs to the page, and a `keepFallback()`-kept fallback stays interactive).
- **An island hydrates against its own server markup.** The element keeps the HTML it connected with; if another script edited the island while it slept (a design-system runtime stripping whitespace, an A/B tool, a translator) and Svelte's walk mismatches on wake, the runtime puts the server markup back and hydrates that (`data-og-healed`, dev warning) instead of re-rendering client-side; only a second failure falls back to Svelte's recovery (`data-og-recovered`). Foreign scripts should still leave `<ogygia-region>` subtrees alone — whatever they added is gone after a heal.
- **A lake on a csr=true (Kit-hydrated) page is adopted, not hydrated.** Kit's pass keeps the lake's SSR element as opaque DOM (its component ships no client JS — not even in Kit's bundle), and everything authored INSIDE the lake stays in ogygia's world: islands and server islands there emit real regions and wake on the runtime. This is how a site header (static markup + `render: 'deferred'` personal holes + a `wake: 'interaction'` search) stays zero-JS-for-anonymous on csr=true pages too: mark the header import `with { wake: 'none' }` in the layout. Caveat: a lake first mounted during a Kit client-side navigation has no server HTML and renders empty (dev warns) — keep chrome lakes in a layout that persists, or serve the route csr=false. A hole rendered OUTSIDE a lake on a csr=true page is Kit-hydrated around; if Kit gives up hydrating the document (`Failed to hydrate`: a component threw) and mounts it fresh, the hole is rendered again with no address — the runtime restores it from the document's holes record (by identity, `data-og-hole`), so it still fetches; a hole rendered only in the browser has no record and keeps its fallback.
...
## Federation v2 — fragments across deploys (EXPERIMENTAL)

One team's app renders inside another team's page, server-side, with security + identity handled. ONE `federate()` per app, in hooks.server.ts (or a `.server.ts` it imports) — NO route files. It returns typed peer handles keyed by name.

```ts
// MFE (the cms app): serve its table as fragments + a widget catalog
import { federate } from 'ogygia/federation';
export const { dash } = federate({
  name: 'cms', key: env.CMS_SIGNING_KEY, base: '/cms', expose: cms_router,
  peers: { shell: { origin: SHELL, key: env.SHELL_PUBLIC_KEY }, dash: { origin: DASH, key: env.DASH_PUBLIC_KEY } },
  widgets: { kpis: { props: ['org'], make: (props, { user }) => region(Kpis, { org: props.org, viewer: user }) } }
});
// Shell: peers + one table entry to mount the whole cms app
export const { cms, dash } = federate({
  name: 'shell', key: env.SHELL_SIGNING_KEY, visitor: () => session(),
  peers: { cms: { origin: CMS, key: env.CMS_PUBLIC_KEY, timeout: 800, cache: { ttl: 30_000 } }, dash: { origin: DASH, key: env.DASH_PUBLIC_KEY } }
});
'/cms/[...rest]': mount(cms),                              // whole app under shell chrome
<Region of={dash.widget('kpis', { org })} />              // a remote WIDGET as a region
```

- **The `peers` map is SYMMETRIC + bidirectional**: `peers.X.key` (X's PUBLIC key(s), a list = rotation) verifies inbound calls + thaw notices FROM X; the top-level `key` (this app's PRIVATE key) signs outbound. That is the whole trust config. `open: true` = an intentionally-unauthenticated endpoint (mTLS/mesh); exposing with no peer keys warns loudly (fail-closed). Keys: `npx ogygia keys <name>` → `<NAME>_SIGNING_KEY`/`<NAME>_PUBLIC_KEY`; read them via `$env/dynamic/private` (NOT `process.env` — Vite may not expose it to server modules).
- **A remote fragment IS a region** — `peer.page(path, dials?)` (a remote ROUTE) and `peer.widget(name, props?, dials?)` (a remote WIDGET) return `Promise<RegionValue>`, rendered `<Region of={…} />`. `render: 'static'` (default) bakes the fragment HTML into THIS render (one buffered signed hop) + adopts its freeze source-tags; `render: 'deferred'` is a hole the browser fetches from the SHELL's OWN signed endpoint (`/og/frag?…`) — the shell derives the visitor's claims server-side at hole time, so the browser chooses nothing (no open-proxy risk, no allowlist). A peer handle is a SERVER value → build the region in a load and render `data.x`. A router load passes it in-process; a Kit `+page.server.ts` load crosses it over the wire (the baseline `transport = { ...ogygia.transport }` hook already handles regions). GOTCHA (internal): a bake-static inline region MUST carry the html on the TOP-LEVEL `html` field (not just `props.html`) or `encode_region` throws "not awaited" — fixed in `federation/peer.ts` bake_static.
- **`mount(peer)`** (from `ogygia/router`) = whole remote app under shell chrome; status/redirects/form-actions translate, a mounted 4xx/5xx renders the MFE's error body WITH the real status. Canary: `mount(v2.pick({ off: cmsV1, on: cmsV2 }))` or `mount((c) => peer)`. Rollout: `when(cmsRollout, mount(cms))`. Stream: `mount(cms, { stream: true | { fallback } })` (GET only). `mount.kit(peer)` from a plain Kit catchall (`export const load = mount.kit(cms).load`).
- **Cross-app THAW (the headline)**: every `freeze.invalidate()` on one app notifies its peers, which drop the frozen pages + cached documents that baked its fragments, and forward up the chain; a new deploy broadcasts a thaw-all on boot (`deploy: 'auto'` default, `'manual'` off). So a publish/deploy on the cms team thaws the shell pages that embedded cms fragments — no config. Provenance rides the wire (`sources` + `build`); the consumer files its frozen page under peer-prefixed tags (`a:<peer>`/`p:<peer>:<path>`/`r:<peer>:<tag>`) so a thaw matches exactly. Pairs with [frozen pages](#frozen-pages-render-on-write).
- **Signing**: Ed25519, every hop; covers method+path+body+claims+audience(target host)+nonce, ±120s window, replay-rejected. Visitor identity rides INSIDE the signature (on-behalf-of), attaches in-process via a Symbol (an HTTP caller can't set a JS symbol), never transits the browser. `user(c)` reads the verified claims in an exposed router's loads/actions/widgets. `__catalog` (`/og/fragment/__catalog`) is the UNSIGNED manifest; `npx ogygia fragments <origin>` → typed stubs, `--check` fails CI on drift.
- Tracing: W3C `traceparent` continuity + per-team `Server-Timing` on every hop. CORS: a foreign island's chunk is a cross-origin `import()` — the MFE's `/_app/immutable/*` MUST send `Access-Control-Allow-Origin` (CDN/proxy; adapter-node does not).
- **`page` does not cross.** On the MFE server `$app/state` is the MFE's page; in the browser a mounted island hydrates inside the SHELL's document and the same read is the shell's page — same URL, but the shell's `route`/`params`/`page.data`. Dev warns once per island. Pass the value as a prop, or read it with a remote function.
- Tracing: W3C `traceparent` continuity + per-team `Server-Timing` on every hop.
- **`page` does not cross.** On the MFE server `$app/state` is the MFE's page; in the browser a mounted island hydrates inside the SHELL's document and the same read (`$app/state` and `$app/stores`) is the shell's page — same URL, but the shell's `route`/`params`/`page.data` (the MFE's seed stops at the fragment boundary). SSR paints the MFE value, hydrate repaints the shell's. Dev warns once per island (`[ogygia] page.data was read inside a mounted MFE island (<entry>, from <origin>)`). Pass the value as a prop, or read it with a remote function. The shell owns the chrome; the MFE's own layouts still render INSIDE the slot — keep them section chrome, or expose a table placed outside the site-header wrap.

## Flags & experiments (ONE primitive, zero client JS)

`import { flag, decide } from 'ogygia/flag'` — a kill switch, rollout, targeting rule, and A/B/n test are ALL `flag()`, decided server-side during SSR (csr=false pages A/B for free). You READ a flag by CALLING it; you branch with `.pick()`; you wire a vendor with one `decide()`. `experiment`/`layer`/`allowOverrides`/`onExposure`/`batchExposures`/`.bucket`/`.on` are GONE — collapsed into these.
...
- **OpenFeature**: `import { openfeature, ofrep } from 'ogygia/flag/openfeature'` (no vendor SDK dependency — you pass your own client). `decide({ source: openfeature(client) })` or `ofrep({ url })`. A source variant not in the declared set is ignored; a slow/down source degrades to the native rule (never gates first byte).
- **Decision order**: `?og-exp=name:variant` override (dev-only unless `decide({ overrides })`) → carried signed claims (federation) → source → layer gate → native rule (targeting/weights/rollout) → control. Anonymous (no visitor sub) → control.
- **Layers (mutual exclusion)**: members sharing `{ layer: 'group' }` split visitors into equal LANES — one sticky hash of (group, visitor) over the SORTED member names picks each visitor's OWNER; non-owners answer control before their own rule runs (two experiments on one surface never share a visitor, so measurements stay attributable). Real exposure = split ÷ member count (50% in a 2-member layer = 25% of traffic). Override/carried/source sit ABOVE the gate.
...
- `regions: { visible: { margin }, preload: 'load' | 'all' | 'none', presets: { name: { render, wake, margin, maxAge, onExpire, revalidate, keep, prefetch } } }` — `preload` (default `'load'`): which islands get modulepreload hints in the SSR HTML; `'all'` hints every island's closure (non-load at low priority), `'none'` hints nothing
...
## Frozen pages (render-on-write)

`ogygia({ freeze: true })` freezes pages: a GET page whose render observed NO personalization (non-default cookie/header/locals/flag reads, cookie writes, set-cookie, non-200, streamed loads, query strings — each named in a dev note) renders ONCE, is stored WHOLE (html + headers; permanent redirects too) and is served as bytes until a publish thaws it — Kit, loads, and Svelte never run on a hit. Stampedes single-flight (N concurrent colds = ONE render). Holes stay live: `render: 'deferred'` regions inside a frozen shell fetch fresh per visitor (their capabilities mint prerender-grade; keep `OGYGIA_SECRET` stable). The identifier is the verb, like `prerender`; prose says "frozen pages".

- Vite config is the switch + policy ONLY: `freeze: true | { ttl, default }` (TTL backstop, ≤24h). Live objects go in hooks.server.ts: `freeze.configure({ store, edge })` from `'ogygia/freeze'`; no configure() = in-process LRU (drop-in survives).
- **Whole-app vs opt-in**: `freeze: true` (or `{ default: true }`) = whole app on, observed-purity refuses personalized pages, opt a route OUT with `export const freeze = false`. `freeze: { default: false }` = OPT-IN: nothing stores unless a page/layout declares `export const freeze = true`. It **cascades like `csr`** (option-file chain, deepest wins, layouts included). `export const freeze` is NOT a Kit page option — ogygia reads it and STRIPS it before Kit (Kit's validators reject non-Kit exports); opting a route in only makes it eligible, the purity check still decides. Enumerates every route with a `+page.*` (svelte / universal / server — redirect-only routes included).
- **Programmatic routes** carry the same flag in table grammar: `page(C, { freeze })`, `layout(name, C, { freeze })`, `routes(table, { freeze })` — page > innermost layout > table > config default. The table registers a matcher at construction; the handle asks it for any request Kit's file router didn't claim (`route.id` null under `sequence(ogygiaHandle(), app.handle)`, or a catch-all `+server.ts`). Keep `ogygiaHandle()` OUTERMOST in the sequence. Endpoint routes and unclaimed paths follow the config default. The router's default `cache-control: no-store` document stamp yields to the verdict on eligible requests (frozen → `public, s-maxage`; refused → `private, no-store`; an explicit page header always wins).
- Invalidation: `freeze.invalidate('/fr/fr/solar/')` (exact URL — the CMS-webhook grammar), `freeze.invalidateWhere({ prefix: '/fr/fr/' })` (subtree = locale/section nuke), `freeze.invalidate(fn, [args])` (declared sources).
...
- **Self-freshening header on a stored page** (SSR'd, no deferred fallback): mark the component `with { render: 'live', wake: 'load' }` INSIDE a small island host — its HTML bakes into the stored bytes, and on served-from-store documents (the handle stamps a doc marker) it revalidates on first mount: a fresh server render with the visitor's cookies, swapped in. Anonymous traffic mints the canonical (default-valued reads store). A live region at page top level OUTSIDE any island bakes once and never revalidates — always give it an island host (dev warns).
- **Validators are page-level**: entries carry a store-time `etag` + `last-modified`, and the handle answers `if-none-match`/`if-modified-since` with a bodiless 304 — CDN revalidations (Akamai prefresh) and browser reloads of stored pages cost zero body bytes. Refused pages with no app cache-control are stamped `private, no-store` (the verdict speaks both directions).
- **Dev never serves or fills the store** — edits always re-render. The verdict still teaches: eligible pages answer `x-ogygia-freeze: would-store`; refusals print one named console note per path.
- **Serve-time stitching** (the zero-flash upgrade over live): `with { render: 'deferred', stitch: 'serve' }` (presets: `stitch: true`). The shell stores once; EVERY serve re-renders the hole server-side with the visitor's cookies and splices it into the stored bytes — personalized in view-source, first response, no JS needed. Stitched pages are per-visitor: `private, no-store`, no validators, edge-bypassed. Fail-open per hole: a failed render ships the fallback and the client fetches as a normal deferred hole. Nested islands inside stitched content wake themselves.
- **Edge stitching (ESI)**: `with { render: 'deferred', stitch: 'edge' }` keeps the EDGE copy: the page stays a normal cacheable frozen entry (`public, s-maxage`, validators), the capture rewrites each edge hole into `<esi:remove>hole</esi:remove><esi:include src="<signed capability>" onerror="continue"/>` and stamps `Surrogate-Control: content="ESI/1.0"`; an ESI CDN (Akamai/Fastly/Varnish — enable ESI on the property) serves the shell from cache and fetches only the include from origin per request (personal first paint, origin renders ONE region). Degrades on its own: where ESI is not processed (direct origin, dev, CloudFront/Cloudflare) the include is inert, the fallback ships and the client fetches as a normal deferred hole. One `serve` hole on a page taints the whole page to origin-serve (its edge holes get server-stitched too). Ladder: browser (`live`) → edge (`stitch: 'edge'`) → origin (`stitch: 'serve'`). GOTCHA: the stitch MODE is part of the wrapper dedupe key (`identity.ts` strategyKey) — without it a `serve` and an `edge` hole of the same component share one wrapper and the whole page flips mode. Local proof: `pnpm demo:esi` boots the fixture behind an ESI-processing akamai emulator (+ a pass-through cloudfront one) and prints the walkthrough; deck S18 (origin) + S19 (through the edge).
...
- SSR emits `<ogygia-region wake|render="defer" when|entry|endpoint …>` + a devalue `<script data-ogygia-props="<fp>">` sidecar (on a Kit page, all island sidecars sit at the END of the body keyed by the region's `data-og-fp`, one per distinct island; elsewhere the sidecar is the next sibling — `runtime/sidecar.ts` finds either). The page seed (`application/ogygia-page`) follows them and ships only when an island on the page reads `$page`; when it ships, island props that are `page.data` subtrees (same object OR a clone) serialize as `["OgygiaSeedRef", path]` into it (seed-refs.ts) — the client copies the node out of the seed. The runtime custom element schedules per `wake`, dynamic-imports the entry URL written **on the element** (Astro-style — the shared runtime never embeds an app-wide island map), parses props, `hydrate()`s in place. `disconnectedCallback` unmounts — SPA swaps clean up for free.
...
- Inside island graphs, `$app/state|stores|navigation` resolve to shims (Kit's client is uninitialized under csr=false); a page seed script provides `page.data`. Don't "fix" the shims away — that pulls Kit's router into island chunks. On csr=true documents the kit-world page thread hands the shims Kit's real `page`, so shared modules read correct data there too.
...
- Federation deferred holes are shell-signed capabilities (the browser picks nothing), so there is no open-proxy surface; `federate({ expose | widgets })` with no peer keys serves unsigned (loud warn — `open: true` only on purpose).
- Federation failover: an origin ARRAY per peer is a replica pool — reads walk it; **POSTs never fail over** (pinned to primary).
...
- Tests, three layers: unit = Vitest `node` project (`pnpm --filter ogygia test:node`, imports dist); island tests in a REAL browser = Vitest browser mode (`test:browser`, Playwright chromium, `test/browser/*.test.ts` — SSR HTML from `setup.ts`, boots the real runtime); e2e = Playwright test runner (`playwright.config.ts`, `e2e/*.spec.ts`, globalSetup builds lib + playground, `webServer` serves it; self-booting suites use worker fixtures in `e2e/fixtures/`).
