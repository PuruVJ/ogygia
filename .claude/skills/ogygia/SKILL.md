...
- `interaction` on a server island = an **on-demand hole**: zero bytes until the visitor shows intent inside it (pointer enters, focus lands, touch/key), then ONE fetch, **morphed** in so what the visitor already opened in the static fallback stays open (no click capture/replay — the fallback handles the gesture natively). Intent radius = `margin` in a preset (`ogygia({ regions: { presets: { menu: { render: 'deferred', wake: 'interaction', margin: '160px' } } } })`, then `with { preset: 'menu' }`): fetch when the pointer comes that close. The shape for a mega menu: fallback = the menu's L1/L2 (today's SSR HTML, byte-identical), hole = the same menu with L3/L4 rendered on the server — no L4 in the initial HTML, no menu JS, no feed download in the browser.
