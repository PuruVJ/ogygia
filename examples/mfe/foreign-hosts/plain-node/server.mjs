 ...
 * Run: node server.mjs   (cms on :5192, serves on :5186)
 */
...
// The v2 signed message: ts.METHOD.pathq.bhash.claims.audience.nonce (audience = the target host,
// claims empty here). Signs AS the shell — the cms trusts SHELL_PUBLIC_KEY as a peer.
const sign_headers = (method, url, body) => {
	...
	const payload = Buffer.from(
		`${ts}.${method.toUpperCase()}.${url.pathname + url.search}.${bhash}..${url.host}.${nonce}`
	);
	return {
		'x-og-ts': ts,
		'x-og-n': nonce,
		'x-og-sig': ed_sign(null, payload, key).toString('base64')
	};
};

const CMS = process.env.CMS_ORIGIN ?? 'http://localhost:5192'; // vite preview binds ::1, so use localhost not 127.0.0.1
const PORT = 5186;
...
http
	.createServer(async (req, res) => {
		...
		// strip the base before forwarding, since federate({ expose }) re-prepends it.
