// Chaos proxy: sits between the shell and the cms (:5187 → :5192). Injects latency/failures,
// counts upstream calls. Control: /__chaos?delay=<ms>&fail=<0..1>  Stats: /__chaos/stats  Reset: /__chaos/reset
// Transport-only (it forwards raw HTTP), so it is federation-version-agnostic — point the shell's
// CMS_ORIGIN at :5187 to drive the cms through it.
import http from 'node:http';

const UP = 'http://localhost:5192';
let delay = 0,
	fail = 0,
	upstream = 0;
...
http
	.createServer(async (req, res) => {
		...
		if (u.pathname === '/__chaos') {
			...
			res.end(JSON.stringify({ delay, fail }));
			return;
		}
		if (u.pathname === '/__chaos/stats') {
			res.end(JSON.stringify({ upstream, delay, fail }));
			return;
		}
		if (u.pathname === '/__chaos/reset') {
			upstream = 0;
			res.end('{}');
			return;
		}
	...
	.listen(5187, () => console.log('chaos proxy on :5187 → :5192'));
