...
export type MarkdownOptions = MarkdownShikiOptions & {
	 ...
	 * content app must not carry the slot. The component VALUES live in `sitekit()`, never here.
	 */
...
export function ogygiaPreprocess(options?: MarkdownOptions): PreprocessorGroup {
	...
	// Element overrides: which tags get wrapped in the ogygia slot (values live in `sitekit()`).
