/**
 * Dependency-free YAML frontmatter parser for ogygia.
 *
 * This REPLACES the `yaml` npm package for the single use-site
 * `src/content/markdown/frontmatter.ts`. It parses the text BETWEEN the `---`
 * fences (the caller strips the fences) and returns a plain JS value — almost
 * always a `Record<string, unknown>`, but a top-level sequence or scalar is
 * also valid. An empty / whitespace-only source returns `{}` (NOT null).
 *
 * ---------------------------------------------------------------------------
