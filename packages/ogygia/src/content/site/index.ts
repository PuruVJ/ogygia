 ...
 * collections) arranged by an `outline()`, projected through the brains `defineSite()` mints.
 ...
 *
 * ```ts
 * import { content, markdown, json, outline, defineSite } from 'ogygia/content';
 *
 ...
 *
 * export const site = defineSite(outline([{ label: 'Guides', items: guides, meta }]), { prevNext: 'graph' });
 * ```
 */
...
export { defineSite, mountBase } from './site.js';
