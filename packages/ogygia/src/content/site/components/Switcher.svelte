<script lang="ts">
	 ...
	 * per axis) all come for free. Options carry ogygia-baked hrefs; selecting one routes through the
	 * ogygia SPA router.
	 *
...
{#if axes?.length}
	<div class="og-switcher">
		{#each axes as ax (ax.axis)}
			<DropdownMenu.Root>
				<DropdownMenu.Trigger class="og-sw-trigger" title={ax.label}>
					<span class="og-sw-value">{ax.current}</span>
					<svg class="og-sw-caret" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m6 9 6 6 6-6" /></svg>
				</DropdownMenu.Trigger>
				<DropdownMenu.Portal>
					<DropdownMenu.Content class="og-sw-menu" sideOffset={6} align="end" aria-label={ax.label}>
						<DropdownMenu.RadioGroup value={ax.current}>
							{#each ax.options as o (o.value)}
								<DropdownMenu.RadioItem
									class="og-sw-opt{o.missing ? ' og-missing' : ''}"
									value={o.value}
									onSelect={() => go(o.href)}
									title={o.missing ? `${o.value} — not translated, falls back` : o.value}
								>
									{#snippet children({ checked })}
										<span class="og-sw-opt-val">{o.value}</span>
										{#if checked}
