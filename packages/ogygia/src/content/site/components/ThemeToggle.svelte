<script lang="ts">
	 ...
	 * `{@html script((k) => { try { var t = localStorage.getItem(k); if (t === 'light' || t === 'dark') document.documentElement.setAttribute('data-theme', t); } catch (e) {} }, 'og-theme')}`
	 */
	...
	let {
		...
		/** localStorage key (default `og-theme`) — match your no-flash head script. */
		key?: string;
		label?: string;
	} = $props();
...
<button type="button" class="og-theme-toggle" aria-label={label} title={label} onclick={cycle}>
	<svg class="og-tt og-tt-system" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linejoin="round" aria-hidden="true">
		<rect x="2" y="4" width="20" height="13" rx="2" /><path d="M8 21h8M12 17v4" />
	</svg>
	<svg class="og-tt og-tt-light" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true">
		<circle cx="12" cy="12" r="4" /><path d="M12 2v2M12 20v2M4.9 4.9l1.4 1.4M17.7 17.7l1.4 1.4M2 12h2M20 12h2M4.9 19.1l1.4-1.4M17.7 6.3l1.4-1.4" />
	</svg>
	<svg class="og-tt og-tt-dark" width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
...
<style>
	...
	:global(:root:not([data-theme])) .og-tt-system,
	:global(:root[data-theme='light']) .og-tt-light,
	:global(:root[data-theme='dark']) .og-tt-dark {
		display: inline;
	}
</style>
