/**
 * `fields` — the blessed frontmatter schema family. `fields.page` validates EXACTLY what the pharos
 ...
 * ```
 *
 * Hand-written Standard Schema (the `string_arg` precedent in `server.ts`) so pharos pulls no
 * valibot/zod dependency. Each returns `{ value }` or `{ issues }`.
 */
import type { SchemaLike } from '../content/index.js';

/** What every pharos surface reads off a page. `title` is required; the rest carry blessed defaults. */
...
const page = schema<PageFields>('ogygia-pharos/page', (d) => {
...
const change_only = schema<ChangeFields>('ogygia-pharos/change', (d) => {
