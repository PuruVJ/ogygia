...
<Dialog.Root bind:open>
	<Dialog.Portal>
		<Dialog.Overlay class="ph-sheet-backdrop" />
		<Dialog.Content
			class="ph-sheet"
			aria-label={label}
			onInteractOutside={(e) => {
		...
		>
			<Dialog.Title class="ph-sr-only">{label}</Dialog.Title>
			<div class="ph-sheet-handle" aria-hidden="true"><span></span></div>
			<div class="ph-sheet-body">{@render children()}</div>
			{#if footer}<div class="ph-sheet-footer">{@render footer()}</div>{/if}
		</Dialog.Content>
	</Dialog.Portal>
</Dialog.Root>
