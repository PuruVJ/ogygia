<script lang="ts">
	 ...
	 * <Shell {site} sidebar={MySidebar} footer={null}>…</Shell>
	 * ```
	 */
...
<div class="ph-shell">
	<!-- First focusable element: a keyboard user can jump the nav straight to the content. Visually
	     hidden until focused (see `.ph-skip`). Targets the `<main>` landmark below. -->
	<a class="ph-skip" href="#ph-main">Skip to content</a>
	{#if header}
		{@const H = header}
		<header class="ph-header"><H {site} /></header>
	{/if}
	<div class="ph-frame">
		...
		{/if}
		<main class="ph-main" id="ph-main" tabindex="-1">{@render children()}</main>
	</div>
	{#if footer}
		{@const F = footer}
		<footer class="ph-footer"><F {site} /></footer>
	{/if}
</div>
