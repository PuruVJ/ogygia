<script lang="ts">
	 ...
	 * `.ph-body` and died on a body swap. The work lives in an ATTACHMENT on a hidden anchor; a
	 ...
	 */
	let { selector = '.ph-body pre' }: { selector?: string } = $props();
	...
	function chrome(anchor: HTMLElement) {
		const root = (anchor.closest('.ph-doc') || anchor.parentElement || document.body) as Element;
		...
		function focus_target(el: HTMLElement) {
			el.tabIndex = -1;
			el.classList.add('ph-target-focus');
			el.focus({ preventScroll: true });
			const off = () => {
				el.classList.remove('ph-target-focus');
		...
		// Heading permalinks (the rehype-added `.ph-heading-anchor`) are plain `<a href="#id">` with no
		...
		const on_click = (e: Event) => {
			...
			const a = t?.closest?.('.ph-heading-anchor');
			if (!a) return;
...
<span class="ph-code-chrome" aria-hidden="true" hidden {@attach chrome}></span>
