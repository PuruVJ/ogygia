<script lang="ts">
	...
	function control(root: HTMLElement) {
		...
		bar.className = 'ph-tabs-bar';
		bar.setAttribute('role', 'tablist');
		const marker = document.createElement('span');
		marker.className = 'ph-tabs-marker';
		marker.setAttribute('aria-hidden', 'true');
		bar.appendChild(marker);
		const btns = labels.map((label) => {
			const btn = document.createElement('button');
			btn.type = 'button';
			btn.className = 'ph-tabs-tab';
		...
		const off1 = on(bar, 'click', (e) => {
			const t = (e.target as Element).closest('.ph-tabs-tab');
		...
		const off2 = roving({ selector: '.ph-tabs-tab', orientation: 'horizontal' })(bar) ?? (() => {});
...
<div class="ph-tabs" data-group={group} {@attach control}>
	<div class="ph-tabs-panels">{@render children()}</div>
</div>
