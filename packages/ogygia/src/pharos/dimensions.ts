 ...
 * coordinate. `class Dimensions` wraps N outlines into ONE `Outline` the same `pharos()` consumes —
 * the coordinate is parsed off the FRONT of the slug (no route changes; the default coordinate is bare).
 *
 * Design decisions (see internal/notes/pharos.md), learned from Starlight / Fumadocs / Docusaurus:
...
/** An `Outline` with the coordinate extras `pharos()` surfaces (switcher, fallback, coordinate). */
...
class Dimensions implements Dimensioned {
	...
	constructor(spec: DimensionsSpec) {
		...
		if (this.#names.length === 0) throw new Error('[ogygia/pharos] dimensions() needs at least one axis.');
		for (const name of this.#names) {
			if (!spec.axes[name].values.includes(this.#def(name))) {
				throw new Error(`[ogygia/pharos] axis "${name}": default "${this.#def(name)}" is not in values.`);
			}
		}
		this.axes = spec.axes;
	}
...
/** Mint a dimensioned outline — the i18n/versioning wrapper `pharos()` consumes like any outline. */
