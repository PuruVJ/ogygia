 ...
 */
import type { LinkRef } from '../content/index.js';
import { href_of, type Outline } from './outline.js';
import type { ReadContext } from './pharos.js';
 ...
 *
 *   pharos({ outline: docs, checks: [links()] })
 *   pharos({ outline: docs, checks: [links({ anchors: false, redirected: 'error' })] })
 */
		...
		if (!has_link_data(entry?.meta)) {
			if (!warned.has(hit.collection)) {
				...
				if (import.meta.env?.DEV) {
					console.warn(`[ogygia/pharos] links() check is on, but '${slug}' comes from a collection whose entries carry no 'meta.links' (only markdown collects them). Link checking is a NO-OP for this corpus — fill 'meta.links' in the format to enable it.`);
				}
			}
			return [];
		}
...
// ── runners (used by pharos: load throws on errors, site.check() returns data) ──
...
export function format_findings(slug: string, findings: Finding[], file: string | undefined): string {
	...
	return `[ogygia/pharos] check failure${findings.length === 1 ? '' : 's'} on '${slug}'${file ? ` (${file})` : ''}:\n${rows.join('\n')}`;
}
