/**
 * artifacts — tier-2 store on Upstash Redis (REST). Serverless-native: no client library, no
 ...
 *
 *   artifacts.configure({ store: upstash({ url: env.UPSTASH_REDIS_REST_URL,
...
export function upstash(config: UpstashConfig): ArtifactStore {
	...
	const command = async <T>(parts: (string | number)[]): Promise<T> => {
		...
		if (!res.ok) {
			throw new Error(`[ogygia] upstash command failed: ${res.status} ${await res.text().catch(() => '')}`);
		}
