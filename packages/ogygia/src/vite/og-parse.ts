...
export type ParseResult = { program: Node | null; ok: boolean };
...
export function parse_module(code: string, id: string): ParseResult {
	try {
		const result = parseSync(id, code) as { program?: Node; errors?: unknown[] };
		if (result.errors && result.errors.length > 0) return { program: null, ok: false };
		return { program: result.program ?? null, ok: !!result.program };
	} catch {
		return { program: null, ok: false };
	}
}
