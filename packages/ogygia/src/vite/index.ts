...
import { routeCsrIsFalse } from './standalone.js';
...
// filename independently and agree.
...
// Build-time-injected route dir (under src/routes) that keeps Kit's client build alive when every
// real route is csr = false: a layout-only node (no +page) contributes `csr !== false` to Kit's
// `skip_client_build` check without creating a servable URL.
const KEEP_CLIENT_DIR = '.ogygia-keep-client';
...
export function ogygia(options: OgygiaOptions = {}): Plugin[] {
	...
	const inject_keep_client_route = (r) => {
		...
		fs.writeFileSync(
			...
			'// Generated by ogygia for the duration of the build (removed afterwards): a layout-only\n' +
				'// node — no +page, so no servable URL — that stops SvelteKit skipping its client build\n' +
				'// when every real route is csr = false. Safe to delete.\n' +
	...
	return [
		{
			...
			config: {
				// Kit discovers routes (`sync.all` → `skip_client_build`) inside its own `config`
				// hook, which is `order: 'pre'` — ours must be `pre` too (plus the documented array
				// position before `sveltekit()`) to have the injected route on disk first.
				order: 'pre',
				handler(config, env) {
					// Kit skips its ENTIRE client build when every route node resolves to
					// `csr === false`. Injecting a keep-client route makes that check always false,
					// so the emitFile piggyback (runtime + island chunks) works with no token
					// `csr = true` page. Kit's postbuild analyse/prerender workers re-run config
					// hooks (SVELTEKIT_FORK) — must not write there: the prerender worker runs
					// after cleanup and would resurrect the dir.
					if (env.command === 'build' && !standalone && !process.env.SVELTEKIT_FORK) {
						inject_keep_client_route(path.resolve(config.root ?? '.'));
					}
		...
		configResolved(config) {
			...
			// Self-heal a keep-client route left behind by a crashed build (harmless — it has no
			// URL — but noisy in the routes tree).
		...
		buildStart() {
			...
			// SAME name independently and agree. The client build always runs — the keep-client
			// route injected in `config` defeats Kit's `skip_client_build` short-circuit.
			if (is_build && !is_ssr) {
				prescan();
				if (!standalone) {
		...
		load(id, options) {
			...
			if (id === RESOLVED(V_RUNTIME_URL)) {
				// dev: the vite dev URL. build: the deterministic SOURCE-content-hashed runtime
				// URL (both build passes compute the same name independently).
				const url = is_dev ? '/@id/__x00__' + V_RUNTIME : RUNTIME_URL_BUILD;
		...
		closeBundle: {
			// `order: 'post'` = after Kit's own closeBundle (the adapter). Kit nests the client
			// build, postbuild workers and prerender inside the SERVER pass's writeBundle, and the
			// workers re-read src/routes via sync.all — the injected route must survive until the
			// very end of the server pass.
			order: 'post',
			handler() {
				if (is_build && is_ssr && !standalone) {
					fs.rmSync(keep_client_dir(root), { recursive: true, force: true });
				}
			}
		}
		},
