/**
 * freeze — CloudFront edge adapter: CreateInvalidation with real SigV4 request signing.
 *
...
function amz_dates(): { datetime: string; date: string } {
	const iso = new Date().toISOString().replace(DASH_COLON_G, '').replace(MILLISECONDS_RE, '');
	return { datetime: iso, date: iso.slice(0, 8) };
}

export function cloudfront(config: CloudfrontConfig): EdgeAdapter {
	const api_base = (config.baseUrl ?? `https://${API_HOST}`).replace(TRAILING_SLASHES_RE, '');
