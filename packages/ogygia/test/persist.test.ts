...
class FakeEl {
	...
	closest(sel: string) {
		if (sel !== `[${PERSIST_ATTR}]`) return null;
		let p: FakeEl | null = this;
		while (p) {
			if (p.attrs[PERSIST_ATTR] != null) return p;
			p = p.parent;
		}
		return null;
	}
	...
	return {
		querySelectorAll(_sel: string) {
			...
			const walk = (el: FakeEl) => {
				if (el.attrs[PERSIST_ATTR] != null) out.push(el);
...
describe('data-ogygia-persist indexing', () => {
	it('indexes top-level keys', () => {
		const nav = new FakeEl({ [PERSIST_ATTR]: 'nav' });
		const foot = new FakeEl({ [PERSIST_ATTR]: 'foot' });
	...
	it('skips nested persist ancestors', () => {
		const inner = new FakeEl({ [PERSIST_ATTR]: 'inner' });
		const outer = new FakeEl({ [PERSIST_ATTR]: 'outer' }).append(inner);
	...
	it('first duplicate key wins', () => {
		const a = new FakeEl({ [PERSIST_ATTR]: 'nav' });
		const b = new FakeEl({ [PERSIST_ATTR]: 'nav' });
	...
	it('ignores empty keys', () => {
		const map = index_top_level_persist(root([new FakeEl({ [PERSIST_ATTR]: '  ' })]));
		expect(map.size).toBe(0);
	});

	it('pairs only keys present on both sides', () => {
		const live_nav = new FakeEl({ [PERSIST_ATTR]: 'nav' });
		const live_only = new FakeEl({ [PERSIST_ATTR]: 'only-old' });
		const next_nav = new FakeEl({ [PERSIST_ATTR]: 'nav' });
		const next_only = new FakeEl({ [PERSIST_ATTR]: 'only-new' });
