...
// Black-box suite for `ogygia site init`. It drives the REAL CLI (the source, run by Node's native
...
function run(dir: string, args: string[]) {
	const res = spawnSync('node', [CLI, 'site', ...args], {
		cwd: dir,
		encoding: 'utf8',
...
describe('site init — svelte.config project', () => {
...
describe('site init — vite.config project', () => {
...
// ── layout safety ───────────────────────────────────────────────────────────
describe('site init — existing layout', () => {
	...
	it('recognises an existing ogygia shell as redeemable and keeps it (no --force needed)', () => {
		const shell = [
			...
			'</script>',
			'',
			'<DocsShell {site} base="" title="My Edited Title">{@render children()}</DocsShell>'
		].join('\n');
		const dir = svelteConfigProject({ 'src/routes/+layout.svelte': shell });
		const { status, out } = run(dir, ['init', '-y', '--no-install']);
		expect(status).toBe(0);
		expect(out).toMatch(/ogygia shell, kept/);
		// the user's edits to their shell layout survive untouched
		expect(read(dir, 'src/routes/+layout.svelte')).toBe(shell);
	});
});

// ── custom + invalid --layout ────────────────────────────────────────────────
describe('site init — --layout', () => {
...
describe('site init — guards', () => {
	...
	it('prints usage for `site` with no subcommand', () => {
		const dir = svelteConfigProject();
		const res = spawnSync('node', [CLI, 'site'], { cwd: dir, encoding: 'utf8' });
		expect(res.status).toBe(0);
		expect(res.stdout).toMatch(/site init/);
	});
});

// ── idempotency + content preservation ───────────────────────────────────────
describe('site init — re-runs', () => {
