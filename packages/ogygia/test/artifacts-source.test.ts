...
import {
	...
	source_tag,
	SOURCE_ID
} from '../src/artifacts/source-runtime.js';
import { set_source_recorder } from '../src/artifacts/capture.js';
import {
	configure,
	reset_for_tests,
	artifact_put,
...
import { upstash } from '../src/artifacts/adapters/upstash.js';
import { cloudflare } from '../src/artifacts/adapters/cloudflare.js';
import { cloudflareKv, type KvNamespaceLike } from '../src/artifacts/adapters/cloudflare-kv.js';
import type { ArtifactEntry, EdgeAdapter } from '../src/artifacts/types.js';

const MARKUP: readonly string[] = ['.svelte'];
const page_entry = (): ArtifactEntry => ({
	kind: 'page',
	html: '<html>x</html>',
...
describe('macros/source — strict position law', () => {
	it('rewrites the legal shape, stamping file#export and injecting the runtime import', () => {
		...
		expect(out).toContain(`import { __og_source } from 'ogygia/artifacts/source';`);
		expect(out).toContain(`export const loadDoc = __og_source("src/lib/server/cms.ts#loadDoc", async (slug) => slug)`);
	});
	...
	it('errors on bare access and on nested calls', () => {
		expect(() => rewrite_source(`const x = [import.meta.og.source];\n`, '/app/x.ts', 'x.ts', MARKUP)).toThrow(/bare/);
		expect(() =>
			rewrite_source(`export const f = () => import.meta.og.source((a) => a);\n`, '/app/x.ts', 'x.ts', MARKUP)
		).toThrow(/outside/);
	});

	it('errors on let / destructuring / wrong arity', () => {
		expect(() => rewrite_source(`export let f = import.meta.og.source((a) => a);\n`, '/app/x.ts', 'x.ts', MARKUP)).toThrow(/const/);
		expect(() => rewrite_source(`export const f = import.meta.og.source();\n`, '/app/x.ts', 'x.ts', MARKUP)).toThrow(/args/);
	});
});
...
describe('invalidate(fn, args) — the reverse index', () => {
	...
	it('evicts exactly the consuming keys and purges them at every edge', async () => {
		...
		await artifact_put('/fr/fr/a', page_entry(), { ttl: 60, tags: [tag] });
		await artifact_put('/en/us/b', page_entry(), { ttl: 60, tags: [tag] });
		await artifact_put('/bystander', page_entry(), { ttl: 60 });

		await invalidate(fn, ['promo']);
		expect(await artifact_get('/fr/fr/a')).toBeNull();
		expect(await artifact_get('/en/us/b')).toBeNull();
		expect(await artifact_get('/bystander')).not.toBeNull();
...
describe('cloudflare edge adapter', () => {
	...
	it('bearer-authed zone purge: files for urls, prefixes for subtrees; cache-tag stamping', async () => {
		...
		expect(edge.headers({ url: '/fr/fr/x', ttl: 60 })['cache-tag']).toBe('p:/fr,p:/fr/fr,p:/fr/fr/x');
