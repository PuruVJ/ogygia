...
| A/B test or staged rollout, zero client JS | `experiment()` / `flag()` from `'ogygia'`, `exp.pick({...})` as a page slot | client-side bucketing |
...
- `ogygia_flags({ dir })` — inventory every `flag()` / `experiment()` call site: reads the build manifest (`node_modules/.ogygia/flags-manifest.json`) when present, else live-scans the source with the real AST collector. Names, kinds, file:line — use it to audit rollouts and find dead flags.
...
- `mount(target, opts)` also accepts a `(c) => FragmentClient` resolver (canary / blue-green per request; keep clients as prebuilt singletons). Buffered by default; `stream: true | { fallback }` flushes shell chrome + fallback first (GET only; a fallback component from a server-only module needs `with { region: 'raw' }`). MFE 4xx/5xx render under shell chrome with the real status; POST follows PRG redirects.
- `kitMount(target, { user?, param? })` — the same mount from a PLAIN Kit catchall (`return { load, actions }` shapes; no ogygia router needed). Upstream errors become Kit `error()` pages.
...
## Experiments & feature flags (zero client JS)

`import { experiment, flag, layer, allowOverrides, onExposure, batchExposures } from 'ogygia'` — assignment is server-side during SSR; csr=false pages A/B test for free.

- `experiment('hero', { variants: ['control','bold'], split: { bold: 20 }, assign?, layer? })` — the FIRST variant is control. Read with `exp.bucket(c)`, or hand `exp.pick({ control: Old, bold: New })` straight to `page()` as the slot. `exp.stamp(c)` → `'hero:bold'` for logging.
- Decision order: `?og-exp=name:variant` override (dev-only unless `allowOverrides(gate)` says otherwise — e.g. QA role) → carried signed claims (federation) → layer gate → `assign(c)` → sticky split (hash of visitor sub) → control. Anonymous (no visitor sub) always gets control from a split.
- `flag('new-checkout', { enabled?: (c) => boolean | undefined, rollout?: 20 })` — `f.on(c)` is the whole read API. `enabled` returning `false` is definitive (rollout can't re-include); `undefined` falls through to rollout; bare `flag('x')` is off for everyone.
- `layer('pricing')` — mutual exclusion: member experiments split the visitor space; a visitor not owned by an experiment sees its control.
- Exposure logging: `onExposure(batchExposures(send, { max: 25, ms: 2000 }))` — fires once per (request, experiment) on the first bucket read; sink errors never break a request; call `.flush()` on shutdown.
- Build inventory: every literal-named `flag()`/`experiment()` call site (AST-resolved 'ogygia' imports, renames + namespaces included) lands in `node_modules/.ogygia/flags-manifest.json` on the client build — diff it in CI to catch dead flags.
