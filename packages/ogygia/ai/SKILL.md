...
- **Validators are artifact-level**: entries carry a store-time `etag` + `last-modified`, and the handle answers `if-none-match`/`if-modified-since` with a bodiless 304 — CDN revalidations (Akamai prefresh) and browser reloads of stored pages cost zero body bytes. Refused pages with no app cache-control are stamped `private, no-store` (the verdict speaks both directions).
- **Dev never serves or fills the store** — edits always re-render. The verdict still teaches: eligible pages answer `x-ogygia-artifact: would-store`; refusals print one named console note per path.
