...
// Usage: pnpm exec playwright test continuity
import { test, check } from './fixtures/index.ts';

test.describe('named wire codec: session-lifetime cart survives SPA nav, merge, tab-isolated', () => {
	test('cart continuity across SPA nav + persistent player + ambient form-field survival + fresh-tab isolation', async ({
		page,
		browser,
		baseURL
	}) => {
		...
		await page.goto('/cart-a', { waitUntil: 'networkidle' });
		...
		await page
			.locator('[data-persist-player]')
			.evaluate((el) => ((el as unknown as { __ogTag: number }).__ogTag = 12345));
		...
		check(
			'player: track is /cart-a before nav',
			(await page.locator('[data-pp-track]').innerText()) === '/cart-a'
		);
		...
		check(
			'A: last item is from page a',
			(await page.locator('[data-sc-last]').innerText()) === 'a-2'
		);
		...
		check(
			'B: navigated (widget shows page b)',
			(await page.locator('[data-sc-page]').innerText()) === 'b'
		);

		// The persisted player is the SAME live node (our tag survived) and never reset its ticks.
		check(
			'player: SAME live node across nav (tag survived)',
			(await page
				.locator('[data-persist-player]')
				.evaluate((el) => (el as unknown as { __ogTag?: number }).__ogTag)) === 12345
		);
		...
		check(
			'player: kept playing through nav (ticks did not reset)',
			ticksAfterNav >= ticksBeforeNav,
			`before=${ticksBeforeNav} after=${ticksAfterNav}`
		);
		// PROP-PUSH: the relocated live player took page B's track (not frozen at page A's first-mount).
		check(
			'player: prop-push updated track to new route (/cart-b)',
			(await page.locator('[data-pp-track]').innerText()) === '/cart-b',
			`track=${await page.locator('[data-pp-track]').innerText()}`
		);
		check(
			'B: cart items SURVIVED navigation (2)',
			(await page.locator('[data-sc-count]').innerText()) === '2',
			`count=${await page.locator('[data-sc-count]').innerText()}`
		);
		check(
			'B: last user item survived (a-2)',
			(await page.locator('[data-sc-last]').innerText()) === 'a-2'
		);
		const stampB = await page.locator('[data-sc-stamp]').innerText();
		check(
			'B: server stamp advanced + merged in (fresh render)',
			stampB !== stampA && Number(stampB) > Number(stampA),
			`A=${stampA} B=${stampB}`
		);
		...
		await page.waitForTimeout(50);
		check(
			'B: same live cart — add continues it (3)',
			(await page.locator('[data-sc-count]').innerText()) === '3'
		);
		check(
			'B: last item now from page b',
			(await page.locator('[data-sc-last]').innerText()) === 'b-1'
		);
		...
		await page.waitForTimeout(250);
		check(
			'A again: cart still has 3 items (continuity round-trip)',
			(await page.locator('[data-sc-count]').innerText()) === '3',
			`count=${await page.locator('[data-sc-count]').innerText()}`
		);
		...
		await page.goto('/formkeep', { waitUntil: 'networkidle' });
		await page.waitForTimeout(150);
		...
		await page.waitForTimeout(50);
		check(
			'form: typed + checked, bind echoes',
			(await page.locator('[data-kf-echo]').innerText()) === 'Ada Lovelace ✓'
		);
		...
		check(
			'form: text field restored',
			(await page.locator('[data-kf-name]').inputValue()) === 'Ada Lovelace',
			`value=${await page.locator('[data-kf-name]').inputValue()}`
		);
		check('form: checkbox restored', await page.locator('[data-kf-sub]').isChecked());
		check(
			'form: bind resynced after restore (echo)',
			(await page.locator('[data-kf-echo]').innerText()) === 'Ada Lovelace ✓',
			`echo=${await page.locator('[data-kf-echo]').innerText()}`
		);
		check(
			'form: data-ogygia-no-keep field NOT restored (starts blank)',
			(await page.locator('[data-kf-otp]').inputValue()) === '',
			`otp=${await page.locator('[data-kf-otp]').inputValue()}`
		);

		// Isolation: a FRESH tab (new context) starts empty — the Keep is per-tab, server never remembers.
		// (`browser.newContext()` does not inherit the config's baseURL the way the `page` fixture does.)
		const fresh = await browser.newContext({ baseURL });
		const fp = await fresh.newPage();
		await fp.goto('/cart-a', { waitUntil: 'networkidle' });
		await fp.waitForTimeout(150);
		check(
			'fresh tab: cart is empty (session-scoped, no server memory)',
			(await fp.locator('[data-sc-count]').innerText()) === '0',
			`count=${await fp.locator('[data-sc-count]').innerText()}`
		);
		await fresh.close();
	});
	...
	test('speculation rules: NEVER in SPA mode', async ({ page }) => {
		await page.goto('/cart-a', { waitUntil: 'networkidle' });
		await page.waitForTimeout(150);
