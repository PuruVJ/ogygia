...
//
//   node e2e/region-mixed.ts http://localhost:3051
...
async function probe(page: Page, pathname: string) {
	...
	return { before, after, dom, errors: errors.filter((e) => !/favicon/.test(e)) };
}

const browser = await chromium.launch();
try {
	// ── csr=false: RegionInside as an island (ogygia hydrates it — unchanged) ───
	const a = await probe(await browser.newPage(), '/region-mixed');
	check('csr=false: RegionInside rendered once', a.dom.regionInside === 1, `count=${a.dom.regionInside}`);
	check('csr=false: inner widget present exactly once', a.dom.widget === 1, `count=${a.dom.widget}`);
	check('csr=false: inner widget SSR value (start=5)', a.before.includes('count is 5'), a.before);
	check('csr=false: inner widget hydrated (2 clicks → 7)', a.after.includes('count is 7'), a.after);
	check('csr=false: the region IS an island here (<ogygia-region> + runtime present)', a.dom.regionEls >= 1 && a.dom.runtimeScript, `els=${a.dom.regionEls} runtime=${a.dom.runtimeScript}`);
	check('csr=false: zero page/console errors', a.errors.length === 0, a.errors[0] ?? '');

	// ── csr=true: RegionInside as a PLAIN component — renders in the Kit tree ───
	...
	const b = await probe(await browser.newPage(), '/region-mixed-kit');
	check('csr=true: RegionInside rendered once (no crash)', b.dom.regionInside === 1, `count=${b.dom.regionInside}`);
	check('csr=true: inner widget present exactly once (single hydration)', b.dom.widget === 1, `count=${b.dom.widget}`);
	check('csr=true: inner widget SSR value (start=7)', b.before.includes('count is 7'), b.before);
	check('csr=true: inner widget hydrated by Kit (2 clicks → 9)', b.after.includes('count is 9'), b.after);
	check('csr=true: ZERO <ogygia-region> — the region rendered inline in the Kit tree', b.dom.regionEls === 0, `count=${b.dom.regionEls}`);
	check('csr=true: NO ogygia runtime script — Kit owns hydration', b.dom.runtimeScript === false, `runtime=${b.dom.runtimeScript}`);
...
if (failures) {
	console.log(`\n${failures} REGION-MIXED CHECK(S) FAILED`);
	process.exit(1);
}
console.log('\nALL REGION-MIXED CHECKS PASSED');
