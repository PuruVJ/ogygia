...
//
//   node e2e/bench.ts "baseline"          # micro-benches only (fast)
//   node e2e/bench.ts "baseline" --build  # also cold-builds docs (adds ~4s)
//
...
const docsRt = runtimeChunk('apps/docs');
const pgRt = runtimeChunk('apps/playground');
console.log(`\n▸ runtime chunk (gzip)`);
...
if (!existsSync(file)) {
	writeFileSync(
		...
		'# ogygia perf checkpoints\n\n' +
			'transformHost µs/call (lower better) · runtime chunk gzip B · docs cold build s. Recorded by `node e2e/bench.ts <label>`.\n\n' +
...
console.log(`\n✓ checkpoint '${label}' appended to internal/notes/perf-checkpoints.md\n`);
