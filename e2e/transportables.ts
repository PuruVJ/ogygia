...
// Usage: node verify/transportables.ts [baseUrl]
...
// ---------- Codec engine (unit-level, no server) ----------
{
	class Box {
		v: number;
		constructor(v = 0) {
	...
	check('client (remember) shares one live instance', c1 === c2 && c1 instanceof Box && c1.v === 7);
...
// ---------- Continuity: named codec (session-lifetime, merge on navigation) ----------
{
	...
	const pageA = reduce_transportable(new Cart(['seed'], 1)) as { t: string; i: string; d: unknown };
	...
	const pageB = reduce_transportable(new Cart(['seed'], 2)) as { t: string; i: string; d: unknown };
	...
	check('named: live wins — user edit survived', liveB.items.length === 2 && liveB.items[1] === 'user-added');
...
// ---------- Continuity: default merge (no merge fn) = pure live-wins ----------
{
	class Draft {
		text: string;
		constructor(text = '') {
	...
	const a = revive_transportable(reduce_transportable(new Draft('server-a')) as never, true) as Draft;
	a.text = 'user typed this';
	const b = revive_transportable(reduce_transportable(new Draft('server-b')) as never, true) as Draft;
	check('default merge: same instance, live wins entirely', b === a && b.text === 'user typed this');
}

// ---------- Continuity: duplicate-id collision guard ----------
{
	class One {
	...
	check('duplicate continuity id warns (collision guard)', /both use continuity id "dupe"/.test(msg), msg.slice(0, 60));
}

// ---------- Browser e2e (/transportable) ----------
...
try {
	...
	const raw = await (await fetch(base + '/transportable')).text();
	const ssr = raw.match(/data-t-count[^>]*>(\d+)</)?.[1];
	check('SSR: server-built instance seeds islands (renders 5)', ssr === '5', `count=${ssr}`);

	await page.goto(base + '/transportable', { waitUntil: 'networkidle' });
	...
	check('write in one island repaints the other', (await count()) === '6', `count=${await count()}`);

	await page.locator('[data-transport-writer] button').click();
	await page.locator('[data-transport-writer] button').click();
	await page.waitForTimeout(50);
	check('cross-island count after 3 writes', (await count()) === '8', `count=${await count()}`);
	check('derived getter tracks shared state', (await page.locator('[data-t-double]').innerText()) === '16');
	...
	check('module-script class: SSR seed', (await widgetHits()) === '3', `hits=${await widgetHits()}`);
	...
	check(
		...
		(await page.locator('[data-transport-srv="server"]').getAttribute('data-is-instance')) === 'true' &&
	...
	check(
		...
		(await page.locator('[data-transport-srv="defer-hydrate"]').getAttribute('data-is-instance')) === 'true'
	);
	await page.locator('[data-transport-writer] button').click();
	await page.waitForTimeout(80);
	check(
		'defer+wake: shares the writer’s live instance (repaints on write)',
		(await page.locator('[data-transport-srv="defer-hydrate"] [data-ts-count]').innerText()) === '9',
...
console.log(`\n${failures === 0 ? 'ALL TRANSPORTABLE CHECKS PASSED' : failures + ' FAILED'}`);
