...
test.describe('hydration, load/idle/visible/media, devalue, SPA', () => {
	...
	test('home: load-strategy hydration + interactivity', async ({ page }) => {
		...
		await counter.click();
		check(
			'home: counter hydrated & interactive',
			(await counter.textContent()).includes('count is 12')
		);
		...
		await sleep(400);
		check(
			'home: visible island NOT hydrated before scroll',
			(await visIsland.getAttribute('data-hydrated')) === null,
			'hydrated=' + JSON.stringify(await visIsland.getAttribute('data-hydrated'))
		);

		// devalue revival: click reveals a client-computed value; island must be hydrated
		const dv = page.locator('[data-devalue]');
		check(
			'home: devalue island Date instanceof true (client)',
			(await dv.locator('[data-date]').textContent()).includes('instanceof Date: true')
		);
		await dv.locator('button').click();
		check(
			'home: devalue island hydrated (button works)',
			(await dv.locator('button').textContent()).includes('revived? true')
		);
		check(
			'home: devalue Map revived (client)',
			(await dv.locator('[data-map]').textContent()).includes('instanceof Map: true')
		);
		check(
			'home: devalue Set revived (client)',
			(await dv.locator('[data-set]').textContent()).includes('instanceof Set: true')
		);
		check(
			'home: devalue BigInt revived (client)',
			(await dv.locator('[data-big]').textContent()).includes('typeof bigint')
		);
		...
		check(
			'home: visible island hydrated AFTER scroll',
			(await visIsland.getAttribute('data-hydrated')) !== null
		);
		check(
			'home: visible island logged on hydrate',
			logs.some((l) => l.includes('visible island hydrated'))
		);
		...
		await page
			.waitForFunction(
				() => {
					const el = [...document.querySelectorAll('ogygia-region')].find(
						(n) => /Same module, visible/.test(n.textContent) // in-browser: cannot hoist
					);
					return el && el.hasAttribute('data-hydrated');
				},
				{ timeout: 3000 }
		...
		await lazyBtn.click();
		check(
			'home: per-use strategy (same module, visible) hydrated & interactive',
			COUNT_100_RE.test(await lazyBtn.textContent())
		);
	});

	// ---------- Media strategy: narrow viewport hydrates ----------
	test('media strategy: narrow viewport hydrates, wide does not', async ({ page, context }) => {
		await page.setViewportSize({ width: 500, height: 700 });
		await page.goto('/', { waitUntil: 'networkidle' });
		await page
			.waitForSelector('ogygia-region[wake*="max-width"][data-hydrated]', { timeout: 3000 })
			.catch(() => {});
		const media = page.locator('ogygia-region[wake*="max-width"]');
		check(
			'home(narrow): media island hydrated when query matches',
			(await media.getAttribute('data-hydrated')) !== null
		);
		await page.close();

		const wide = await context.newPage();
		await wide.setViewportSize({ width: 1200, height: 700 });
		await wide.goto('/', { waitUntil: 'networkidle' });
		await sleep(500);
		const media2 = wide.locator('ogygia-region[wake*="max-width"]');
		check(
			'home(wide): media island NOT hydrated when query does not match',
			(await media2.getAttribute('data-hydrated')) === null
		);
		await wide.close();
	});
	...
	test('remote functions in islands: SSR-resolved data + islands hydrate', async ({ page }) => {
		...
		check(
			'data: mode (a) resolved greeting present (SSR)',
		...
		await sleep(400);
		const anyHydrated = await page.locator('ogygia-region[data-hydrated]').count();
		check(
			'data: islands hydrate (custom elements connect)',
			anyHydrated >= 1,
			`hydrated=${anyHydrated}`
		);
	});
});
