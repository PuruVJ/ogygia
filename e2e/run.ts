...
const CHECKS: Array<[file: string, needsServer: boolean, note: string]> = [
	...
	['artifacts.ts', false, 'ARTIFACTS (render-on-write): store/serve/invalidate + prefix nuke + edge emulators (real adapter signing) + stampede single-flight + holes prerender-grade (self-building fixture)'],
	['artifacts-dev.ts', false, 'ARTIFACTS dev leg: vite dev never serves/fills the store (edits stay live) — would-store teaching header, per-request renders (self-booting fixture dev server)'],
