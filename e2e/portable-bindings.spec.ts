...
// Usage: pnpm exec playwright test portable-bindings
...
// transform-level probes
const REWRITE_A_RE = /import A from "virtual:ogygia\/region\//;
const STATIC_TAG_ERROR_RE = /never used as a static/;
const HYDRATE_VISIBLE_RE = /__hydrate: "visible"/;
const HYDRATE_LOAD_RE = /__hydrate: "load"/;
const REWRITE_RAWX_RE = /import RawX from "virtual:ogygia\/region\//;
const REWRITE_WAKEX_RE = /import WakeX from "virtual:ogygia\/region\//;
// browser probes
const GREETING_RE = /loading greeting|Portable/i;
// build-output probes
const IMMUTABLE_PREFIX_RE = /^\/_app\/immutable\//;
const LEADING_DOT_SLASH_RE = /^\.\//;
const FROM_SPEC_RE = /from\s*["']([^"']+)["']/g;
const CLASSIC_THIN_RE =
	/^import\{[^}]+\}from"\.\/chunks\/[^"]+";(var [^=]+=[^;]+;)?export\{[^}]+as default\};?$/;
const NODE_CSS_RE = /^\d+\..*\.css$/;
const OG_SHELL_RE = /\.og-shell\b/;
const OG_CSIDE_RE = /\.og-cside\b/;
const KIT_NODE_RE = /"\/kit":\[(\d+)/g;
...
const PORTABLE_NODE_RE = /"\/\(spa\)\/portable":\[(\d+)/g;
const OG_REGION_HASH_RE = /og-region\.([0-9a-f]+)\.js/;

const ROOT = '/app';
/** The mock compiler ctx both transform-level tests share (`...CTX_EXTRA` first so overrides win). */
const make_ctx = () => ({
	...CTX_EXTRA,
	root: ROOT,
	libDir: '/app/src/lib',
...
test.describe('static/dynamic/list bindings + shared-entry dedupe + .ts mixed raw/wake', () => {
	test('Transform: dedupe identity (unit-level, no server)', () => {
...
`;
	...
	test('Transform: `.ts` registry — mixed region:raw + wake:, held vs placed identities', () => {
		const ctx = make_ctx();
		// `.ts` held ids: strategy 'held', its baked schedule (`region:raw` / `region:visible`).
		const held_id_of = (rel: string, hydrate?: string) => {
...
`;
		...
		check(
			'transform(.ts mixed): three distinct islands (duplicate wake deduped)',
			islands.length === 3,
			`count=${islands.length}`
		);

		const rawX = byId.get(held_id_of('src/lib/X.svelte'));
		const wakeX = byId.get(held_id_of('src/lib/X.svelte', 'visible'));
		const loadY = byId.get(held_id_of('src/lib/Y.svelte', 'load'));
		...
		check(
			...
			!!wakeX &&
				...
				HYDRATE_VISIBLE_RE.test(String(wakeX.bindingSsrSource)) &&
		...
		check(
			...
			!!loadY &&
				loadY.server === true &&
				loadY.held === true &&
				HYDRATE_LOAD_RE.test(String(loadY.bindingSsrSource))
		);
		// The SAME component marked raw vs wake → two DISTINCT islands, not a collision.
		check(
			'transform(.ts mixed): same component raw vs wake → distinct islands',
			!!rawX && !!wakeX && rawX.id !== wakeX.id
		);
		...
		check(
			...
			REWRITE_RAWX_RE.test(rt.code) && REWRITE_WAKEX_RE.test(rt.code)
		);
	...
	test('Browser e2e', async ({ page }) => {
		await page.goto('/portable', { waitUntil: 'networkidle' });
		...
		check(
			'static: hydrated click',
			(await staticBtn.textContent())?.includes('count is 11') ?? false
		);
		...
		check(
			'list[0] interactive',
			(await listBtns.nth(0).textContent())?.includes('count is 2') ?? false
		);
		check(
			'list[1] interactive',
			(await listBtns.nth(1).textContent())?.includes('count is 3') ?? false
		);
		...
		check('fill: fallback or greeting present', GREETING_RE.test(greeted), greeted.slice(0, 80));
	});

	test('Client build: Counter facade dedupe (Rolldown thin facades)', () => {
		...
		if (!fs.existsSync(clientDir)) {
			...
			const resolve_imp = (fromFile: string, spec: string) => {
				if (spec.startsWith('/')) {
					const trimmed = spec.replace(IMMUTABLE_PREFIX_RE, '');
					return trimmed;
				}
				...
				return joined.replace(LEADING_DOT_SLASH_RE, '');
			};
			const reaches = (rel: string, seen = new Set<string>()): boolean => {
				...
				for (const m of code.matchAll(FROM_SPEC_RE)) {
					if (reaches(resolve_imp(key, m[1]), seen)) return true;
				}
				return false;
			};
			const facades = fs
			...
			const classic_thin = (code: string) => CLASSIC_THIN_RE.test(code.trim());
			const reaches_marker = (rel: string, marker: string, seen = new Set<string>()): boolean => {
				const key = rel.split(path.sep).join('/');
				if (seen.has(key)) return false;
				seen.add(key);
				const abs = path.join(clientDir, key);
				...
				if (code.includes(marker)) return true;
				for (const m of code.matchAll(FROM_SPEC_RE)) {
					if (reaches_marker(resolve_imp(key, m[1]), marker, seen)) return true;
				}
				return false;
			};
			const clockFacades = facades.filter((f) => reaches_marker(f, 'data-clock-island'));
			const thinClocks = clockFacades.filter((f) =>
				classic_thin(fs.readFileSync(path.join(clientDir, f), 'utf-8'))
			);
			check(
			...
			if (fs.existsSync(docsClient)) {
				...
				const nodeCss = fs.existsSync(assetsDir)
					? fs.readdirSync(assetsDir).filter((f) => NODE_CSS_RE.test(f))
					: [];
				const shellCss = nodeCss.find((f) => {
					const code = fs.readFileSync(path.join(assetsDir, f), 'utf-8');
					return OG_SHELL_RE.test(code) && OG_CSIDE_RE.test(code);
				});
				check(
			...
			if (fs.existsSync(nodesDir) && appEntry) {
				...
				for (const m of appCode.matchAll(KIT_NODE_RE)) {
				...
				const portableNode = [...appCode.matchAll(PORTABLE_NODE_RE)].map((m) => m[1])[0];
				if (portableNode) {
			...
			if (fs.existsSync(depsPath)) {
				...
				const portableEntry =
					'/_app/immutable/og-region.' +
					(counterFacades[0]?.match(OG_REGION_HASH_RE)?.[1] ?? '') +
					'.js';
