...
//
// Usage: OGYGIA_DEVTOOLS=1 node e2e/run.ts --only=devtools   (after a devtools-on build)
...
try {
	...
	if (!(await devtools_present(page))) {
		skip('devtools build', 'window.__ogygia_devtools absent — served build has devtools OFF (tree-shaken)');
		console.log(results.join('\n'));
		console.log('\nDEVTOOLS CHECKS SKIPPED (build has devtools off)');
		await browser.close();
		process.exit(0);
	}
	...
	check('server: events present in the unified stream (realm=server)', serverEvents.length > 0, `${serverEvents.length} server events`);
	const serverRendered = serverEvents.filter((e) => e.name === 'server.region.rendered');
	check('server: server.region.rendered (fp + mode + propsBytes)', serverRendered.length > 0 && typeof serverRendered[0].fp === 'string' && typeof serverRendered[0].propsBytes === 'number', `${serverRendered.length} rendered, sample propsBytes=${serverRendered[0]?.propsBytes}`);
	check('server: seed injected event present', serverEvents.some((e) => e.name === 'server.seed.injected'));
	...
	if (launcher) {
		...
		check('lens: one overlay box per rendered region', boxes === regionCount, `${boxes} boxes / ${regionCount} regions`);
		...
		check('lens: hover tooltip fuses server props + client hydrate (by fp)', /fp/.test(tip) && /props/.test(tip) && /server-rendered/.test(tip), tip.slice(0, 80));
		...
		check('bytes: table opens with rows + a page total', !!led && led.rows > 0 && /kB/.test(led.total), JSON.stringify(led));
		...
		const wire = await page.evaluate(() => {
			...
			const rows = win.querySelectorAll('tbody tr').length;
			return { rows, hasProps: /props payload/.test(text), hasTotal: /data across the wire/.test(text) };
		});
		check('wire: shows server crossings (props payload) + a data total', !!wire && wire.rows > 0 && wire.hasProps && wire.hasTotal, JSON.stringify(wire));
		...
		const hubEmpty = await page.evaluate(() => {
			const win = document.querySelector('[data-og-win]');
			return win ? /hub inspector/.test(win.textContent || '') && /no hub activity/.test(win.textContent || '') : false;
		});
	...
	check('load island: region.hydrate.done arrived (with ms)', typeof loadHydrated?.ms === 'number', `ms=${loadHydrated?.ms}`);
	...
	const serverFps = new Set(allNow.filter((e) => e.realm === 'server' && e.name === 'server.region.rendered').map((e) => e.fp).filter(Boolean));
	const clientFps = new Set(allNow.filter((e) => e.realm === 'client' && e.name === 'region.hydrate.done').map((e) => e.fp).filter(Boolean));
	const correlated = [...clientFps].filter((fp) => serverFps.has(fp));
	check('spine: a server render fp matches a client hydrate fp (same region, both realms)', correlated.length > 0, `correlated ${correlated.length} (server ${serverFps.size}, client ${clientFps.size})`);
	...
	check('nav: finish carries a ms timing', typeof navFinish?.ms === 'number', `ms=${navFinish?.ms}`);
	const afterNav = await events(page);
	const navStart = afterNav.find((e) => e.name === 'nav.start' && (e as any).to === '/');
	check('nav: nav.start emitted before finish', !!navStart && navStart.seq < (navFinish as DtEvent).seq);
	...
	// ── Nav tab: the panel shows the last nav's per-region decisions + timing ──
	{
		...
		check('nav-lab: tab shows the last nav decisions + timing', !!nav && nav.rows > 0 && nav.hasDecisions && nav.hasTiming, JSON.stringify(nav));
	}
	...
	const trace = await page.evaluate(() => {
		...
		return { kind: t.kind, version: t.version, count: t.events.length, json: JSON.stringify(t).length };
	});
	check('trace: kind + version + non-empty + JSON-serializable', trace.kind === 'ogygia-devtools-trace' && trace.version >= 1 && trace.count > 0 && trace.json > 0, `${trace.count} events, ${trace.json}B`);
	...
	if (tlLauncher) {
		...
		check('timeline: tab plots client wake/hydrate events on the axis', !!tl && tl.dots > 0 && /timeline/.test(tl.head), JSON.stringify(tl));
	}
	...
	// ── Hub tab, POPULATED: /transportable shares one wired instance across islands → reunions ──
	{
		...
		await hp.goto(base + '/transportable', { waitUntil: 'load' });
		await hp.waitForTimeout(500);
		if (await hp.evaluate(() => typeof (window as any).__ogygia_devtools !== 'undefined')) {
			await hp.click('[data-og-panel-toggle]');
			await hp.click('[data-og-tab="hub"]');
			await hp.waitForTimeout(250);
			const hub = await hp.evaluate(() => {
				const win = document.querySelector('[data-og-win]');
				if (!win) return null;
				const rows = win.querySelectorAll('tbody tr').length;
			...
			check('hub: /transportable shows a shared instance reunited across islands', !!hub && hub.rows > 0 && hub.reunions, JSON.stringify(hub));
		} else {
			skip('hub: populated', 'devtools off on this build');
		}
		await hp.close();
	}
} finally {
...
console.log(results.join('\n'));
console.log(`\n${failures === 0 ? 'ALL DEVTOOLS CHECKS PASSED' : failures + ' FAILED'}`);
