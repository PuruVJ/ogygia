# freeze — frozen pages (design draft)

> **NAMING: SETTLED — `freeze` (user pick, 2026-09-02, after `artifacts` shipped and was
> reopened).** A frozen page renders one time, is stored whole, and is served as bytes until a
> publish thaws it. The word already lived in the docs ("the shell freezes", "a frozen shell")
> before it became the API, and `Object.freeze` hands every JS dev the mental model: immutable
> until replaced. Kit-grammar fit: `export const freeze = true` beside `prerender`/`ssr`/`csr`.
> Rejection trail, for the record: *materialize* (confusing noun), *edition/shelf/pin/once* (too
> non-technical), *derived/projection* (don't describe the mechanism), *render\** (「render」implies
> client rendering), *snapshot* (Kit page export collision), *cache* (too common, wrong mental
> model), *artifacts* (a build-output word for a runtime thing — shipped, then reopened),
> *published* (a CMS-workflow word in the user's field), *isr* (Vercel's; its default flavor is
> timer SWR, and it hides the two tiers + stitching), *memo* (reads component-level, React.memo),
> *ssr = 'once'* (overloads a Kit option — vetoed on principle: never overload an existing concept).
> The identifier is the verb, like `prerender`; prose says **frozen pages**.
...
> caches no data — it is eviction bookkeeping: it answers "when must this stored FREEZE die".
> Layer map (bcms): Akamai caches bytes at the edge (no invalidation today) · freeze cache
...
ONCE per data change and is stored as an freeze; requests concatenate bytes. Cost moves from
...
// virtual:ogygia/freeze-config (the profiler-config pipe). This is the whole adoption:
ogygia({ freeze: true })            // tier-1 in-process store by default
...
- **Invalidation — three sources, two of them free:**
  ...
  3. Explicit: `freeze.invalidate(tag)` from any server code (CMS webhook endpoints).
- **Per-component opt-out = the existing dial.** Inside an freeze page, a component that
...
| Tier | Where | When |
| --- | --- | --- |
| 1 (default) | in-process bounded LRU + optional disk (`.ogygia/freeze/`) | single-instance adapter-node; zero infra |
...
urlPath — precise per publish). Config: `freeze: { store, edge: [akamai(creds),
...
- `FreezeStore { get(key), put(key, Entry, {ttl}), evict(url), evictWhere({prefix}) }`
...
per-request (default). Freeze are the missing rung: **write-time** — invalidated by data,
never by clock or traffic. R6 explicitly stopped short of page-granular compute ("off-limits
compiler"); freeze sidestep that: the page stays monolithic, stored whole, with holes.
...
TTL. So freeze here are not an optimization, they are **the missing correctness layer**:
Builder publish webhook → `freeze.invalidate('builder:<model>:<urlPath>:<locale>')` →
...
freeze (and for their existing Akamai cache!) to stay correct. `render: 'live'` regions are
...
3. **Auto-tags from app-recorded render context, not only `depends()`.** Their loader already
   ...
   content identity get tagging for free via a tiny `freeze.tag()` (or an adapter reading
...
5. **csr-agnostic.** Freeze storage is a render-layer concern; the `(pes)` csr=true subtree's
...
- **`og.source()` — PULLED BACK INTO v1 (user, 2026-09-01: "we need it done now as well") and
  ...
  injects `ogygia/freeze/source` (isomorphic, client-safe: fnv1a from runtime/fingerprint —
  ...
  `evictByTag(tag) → keys` answers `freeze.invalidate(fn, args)` — evicted at origin AND
...
- **L1 — purity as a compile-time proof.** AST-walk the loads + their import graphs (the flag
  collector's machinery) → per-route manifest (`virtual:ogygia/freeze-manifest`, the
...
- **L2 — `import.meta.og.source()`: declared sources, typed string-free invalidation.**
  ...
  freeze.invalidate(loadBuilderContent, ['landing-pages', { urlPath, locale }]);
  ```
...
- **L3 — content-addressed renders (the vanish).** The wire law makes a region's serialized
  ...
  **render-by-hash** for the page shell — run loads (data always fresh), hash the data, look up
  ...
  NOTHING to invalidate. True load-skipping freeze then shrink to an opt-in crust on
...
- **Fragments (MFE).** Each fragment URL is its OWN freeze key — hosts and fragments
...
- **A/B testing (flags).** A flag read during a render personalizes the page → v1 adds a seam in
  ...
  **v2: per-variant freeze** — assignments are a BOUNDED vary dimension (unlike users), so
...
**Lanes 1–2 — the store contract.** `FreezeStore` is four calls (get / put / evict /
...
counts REAL renders per route (`/__freeze/state`) — the strongest assertion in the harness
...
- **live self-freshening (S13, SHIPPED mid-demo)**: the "complex cookie-reading Header, SSR'd,
  ...
  `<meta name="ogygia-freeze">` (stamped at serve time on hit/join, never on the fresh
...
- **og.source precision (S12)**: three pages across two locales embed one shared doc through a
  ...
  reach precisely — and a non-consumer bystander's freeze survives
...
- **DEV never serves/fills the store** — an edited page must always re-render (HMR truth beats
  ...
  `x-ogygia-freeze: would-store`; refusals print ONE named console note per path. Locked by
  `e2e/freeze-dev.ts` (self-booting fixture dev server).
- **Validators at the FREEZE level** (never in edge adapters — they're the render's identity,
...
- **Devtools**: one `server.freeze` event (op: hit/join/stored/skip/invalidate/
...
- **SERVE-TIME STITCHING — SHIPPED as its own round (user decisions: `stitch` on the deferred
  family · fail-open per hole · edge bypass).** `with { render: 'deferred', stitch: 'serve' }`
  inline (import-attr values are strings by grammar; presets take `stitch: true`) — errors on
  non-deferred use. The emitted hole carries a `stitch` attribute; the capture flags the entry
  (`stitch: true`) and flips its nature: shell stores once, but EVERY serve is per-visitor —
  `private, no-store`, NO validators (a 304 would skip the re-stitch), NO edge instructions.
  On every serve (stored/hit/join) the handle finds each hole (balanced scanner in
  freeze/stitch.ts — fallbacks can nest regions), re-renders it through the SAME
  `#render_capability` MAC-verified path the batch endpoint uses (visitor cookies ride the
  event), and splices `<div style="display:contents" data-og-stitched>` in place of the region
  wrapper — nested islands inside become TOP-LEVEL regions that wake themselves (no ancestor to
  wait on). Fail-open PER HOLE: a null/timed-out render keeps the fallback + the client fetch.
  Personalized view-source in the first response, shell render count still 1 — deck S15.
...
- **Page-level SWR (revalidate-on-load)** — the no-write-hooks fallback: when the cached page
  ...
  freeze where invalidation can't be owned. Decide: worth a `page`-level dial, or is
...
## Edge stitching (ESI) — BUILT 2026-09-02

`stitch: 'edge'` is the third rung of the stitch ladder: browser (`render: 'live'`) → edge
(`stitch: 'edge'`) → origin (`stitch: 'serve'`). The user's Akamai chat framed it: Akamai caches
the WHOLE html today; `serve` would forfeit that edge copy. ESI (Akamai/Fastly/Varnish) keeps it.

- Compiler: `stitch: 'serve' | 'edge'` inline (presets also `true` = serve); the value rides
  `__stitch` → Region.svelte emits `stitch="serve|edge"` on the hole element.
- Capture (`freeze/stitch.ts` `stitch_modes` + `esi_rewrite`): one `serve` hole taints the page
  (per-visitor, `private, no-store`, origin re-splice); ONLY-edge pages stay normal cacheable
  entries — each edge hole → `<esi:remove>hole</esi:remove><esi:include src="<signed
  capability>" onerror="continue"/>`, `Surrogate-Control: content="ESI/1.0"`, validators over the
  rewritten bytes, edge headers as usual. Serve path = a plain hit (no `#serve_stitched`).
- Degrade: where ESI isn't processed (dev, direct origin, CloudFront/Cloudflare) `<esi:remove>`'s
  children render (the fallback) and `<esi:include>` is inert → the client fetches the hole as a
  normal deferred hole. Nothing breaks; the personalization just moves back to the browser leg.
- Edge config is the app's job: enable ESI processing on the property and forward the viewer's
  cookies on the include subrequest (the capability is prerender-grade; session sealing is
  whatever `sessionCookie` says).
- Deck S18 (`/fr/fr/esi`): stored → public s-maxage + Surrogate-Control + etag + esi tags; cookied
  second serve = plain hit with the same bytes; the include src fetched from origin with the cookie
  renders the personal header; render count stays 1. Units: `test/freeze-esi.test.ts`.
- Deck S19 + `pnpm demo:esi` (`e2e/demo/esi.ts`): the akamai EMULATOR now processes ESI like the
  real edge (strips `<esi:remove>`, fetches each include from origin WITH THE VIEWER'S COOKIE,
  resolved against the page url, per serve; the cached copy stays the raw shell; `x-edge-akamai-esi:
  assembled` + `esi` count in `/__edge/state`) — a cookied request is an edge HIT that says
  "Hello, Puru" while origin's render count stays 1; the cloudfront personality passes the tags
  through (the degrade path). Two bugs this shook out: (1) the stitch MODE must be in the wrapper
  dedupe key (strategyKey) or `/stitched` and `/esi` share one wrapper and the whole page flips
  mode; (2) the hole endpoint is emitted PAGE-RELATIVE (`../../__ogygia__`) — `esi_rewrite(html,
  page_path)` resolves the include src to an absolute path so any edge can fetch it.
