<script lang="ts">
	/**
	 * The svelte.dev hero — headline, CTA, and the compiler machine. Ported from
	 * apps/svelte.dev/src/routes/_home/Hero.svelte (MIT), same assets, same imagetools pipeline,
	 * same layered gradients (light + dark). Pure SSR — zero JS.
	 */
	// @ts-expect-error — imagetools query import
	import MachineDesktop from './svelte-machine-desktop.png?w=1200;2000;2800&format=avif;webp;png&as=picture';
	// @ts-expect-error — imagetools query import
...
<div class="hero">
	...
	<picture class="machine">
		...
		<img alt="The Svelte compiler packaging up your component code" src={MachineMobile.img.src} fetchpriority="high" />
	</picture>
</div>
...
<style>
	.hero {
		...
		:global(:root:not(.light)) &::before {
			@media (prefers-color-scheme: dark) {
				background:
					linear-gradient(to bottom, transparent, var(--sk-bg-1)),
					radial-gradient(64.14% 72.25% at 47.58% 31.75%, hsl(209deg 6% 47% / 52%) 0%, hsla(0, 0%, 100%, 0) 100%),
					linear-gradient(92.4deg, hsl(210, 7%, 16%) 14.67%, hsl(0deg 0% 0% / 48%) 54.37%, hsla(207, 22%, 13%, 0.62) 92.49%),
					linear-gradient(0deg, hsl(204, 38%, 20%), hsl(204, 10%, 90%));
			}
		}
		:global(:root.dark) &::before {
			background:
				linear-gradient(to bottom, transparent, var(--sk-bg-1)),
				radial-gradient(64.14% 72.25% at 47.58% 31.75%, hsl(209deg 6% 47% / 52%) 0%, hsla(0, 0%, 100%, 0) 100%),
				linear-gradient(92.4deg, hsl(210, 7%, 16%) 14.67%, hsl(0deg 0% 0% / 48%) 54.37%, hsla(207, 22%, 13%, 0.62) 92.49%),
