 ...
 * → section labels), so `defineSite({ outline: docs })` arranges it with zero extra config. `defineSite()` mints
...
/** The site: the `docs` collection auto-arranged by convention (`folder()` supplies order + `+meta.json`
