<script lang="ts">
	// ISLANDS EDITED WHILE THEY SLEEP. After load, this page defines the `<foreign-strip>` custom
	// element that SpacedCounter renders inside each island: its connect reaction strips every
	// whitespace text node of the island around it and leaves a stray comment — what a customer's
	// design-system runtime did to a whole header while the islands in it slept, and the shape any
	// foreign script (A/B tool, translator) produces. Svelte's hydration walk would then discard the
	// server DOM and re-render the island client-side, and the click that woke the interaction
	// island would be replayed onto a dead node. The runtime repairs the island toward the server
	// markup it kept (through the morph, so the foreign element is NOT re-created — re-inserting it
	// would run its reaction again) and hydrates that. e2e/island-foreign-edit.spec.ts.
...
<svelte:head>{@html script(define_foreign_strip)}</svelte:head>
