 ...
 */
// Fully inferred: `Data` from `docSchema`, `Meta` ({ headings }) from `mdsvex`. No annotation.
export const docs = content({
	// The source IS the format now: mdsvex wraps the glob, `id` gives the clean slugs.
	loader: mdsvex(import.meta.glob('../content/docs/**/+doc.svx', { eager: true }), { id: docId }),
	schema: docSchema,
